...
import com.vsiwest.kit.view.*;
import com.vsiwest.lockpick.*;
import com.vsiwest.usecase.*;
...
//        UIManager.setLookAndFeel(UIManager.getCrossPlatformLookAndFeelClassName());
    ...
    private static ArrayList<AbstractAction> createReportActions(final JDesktopPane desktop) {
        ...
        jfm.add(new AbstractAction("BirdCage") {
            public void actionPerformed(ActionEvent e) {
                DefaultMutableTreeNode rootNode = new DefaultMutableTreeNode("error");
                try {
                    final BoxFileTreeView fileTreeView = new BoxFileTreeView();
                    rootNode = BoxFileTreeView.createMultiTree(null);
                } catch (Exception e1) {
                    e1.printStackTrace();
                }
                final JInternalFrame internalFrame = new JInternalFrame();
                try {
                    prepIFrame(internalFrame, "BoxTree", new JScrollPane(new JTree(rootNode)), new Rectangle(80, 20, 200, 600));
                } catch (PropertyVetoException e1) {
                    e1.printStackTrace();  //TODO: verify
                }
        ...
        jfm.add(new AbstractAction("Podcasters") {
