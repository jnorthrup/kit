...
public class TypesWriter {
    ...
    private void writeUINT32(int val, int off) {
        if ((off + 4) > arr.length)
            resize(off + 32);
    ...
    private void writeByte(int v, int off) {
        if ((off + 1) > arr.length)
            resize(off + 32);
    ...
    public void writeCipherList(CipherEntry[] ciphers) {
        ArrayList<String> list = new ArrayList<String>();
        for (CipherEntry cipher : ciphers) {
            String type = cipher.getType();
            list.add(type);
        }
        writeNameList(list.toArray(new String[]{}));
