...
class TransportConnection {
    ...
    private int send_seq_number = 0;

    private int recv_seq_number = 0;

    private CipherInputStream cis;

    private CipherOutputStream cos;

    private boolean useRandomPadding = false;

    /* Depends on current MAC and CIPHER */

    private MAC send_mac;

    private byte[] send_mac_buffer;

    private int send_padd_blocksize = 8;

    private MAC recv_mac;

    private byte[] recv_mac_buffer;

    private byte[] recv_mac_buffer_cmp;

    /* won't change */

    private final byte[] send_padding_buffer = new byte[256];

    private final byte[] send_packet_header_buffer = new byte[5];

    private final byte[] recv_padding_buffer = new byte[256];

    private final byte[] recv_packet_header_buffer = new byte[5];

    private boolean recv_packet_header_present = false;
    ...
    public void changeRecvCipher(BlockCipher bc, MAC mac) {
        ...
        int recv_padd_blocksize = bc.getBlockSize();
    ...
    private void sendMessage(byte[] message, int off, int len, int padd) throws IOException {
        if (padd < 4) padd = 4;
