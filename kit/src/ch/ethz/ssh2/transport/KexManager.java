...
class RefUtils<T extends Comparable> {
    public T getFirstMatch(T[] client, T[] server) throws NegotiateException {
        ...
        for (T aClient : client) {
            for (T aServer : server) {
...
public class KexManager {
    ...
    private static boolean compareFirstOfNameList(String[] a, String[] b) {
        if (a == null || b == null)
            throw new IllegalArgumentException();
    ...
    private NegotiatedParameters mergeKexParameters(KexParameters client, KexParameters server) {
        ...
        try {
            np.kex_algo = (new RefUtils<String>().getFirstMatch(client.kex_algorithms, server.kex_algorithms));

            np.server_host_key_algo = (new RefUtils<String>().getFirstMatch(client.server_host_key_algorithms, server.server_host_key_algorithms));

            np.enc_algo_client_to_server = new RefUtils<CipherEntry>().getFirstMatch(client.encryption_algorithms_client_to_server, server.encryption_algorithms_client_to_server);
            np.enc_algo_server_to_client = new RefUtils<CipherEntry>().getFirstMatch(client.encryption_algorithms_server_to_client, server.encryption_algorithms_server_to_client);

            np.mac_algo_client_to_server = (new RefUtils<String>().getFirstMatch(client.mac_algorithms_client_to_server, server.mac_algorithms_client_to_server));
            np.mac_algo_server_to_client = (new RefUtils<String>().getFirstMatch(client.mac_algorithms_server_to_client, server.mac_algorithms_server_to_client));

            np.comp_algo_client_to_server = (new RefUtils<String>().getFirstMatch(client.compression_algorithms_client_to_server, server.compression_algorithms_client_to_server));
            np.comp_algo_server_to_client = (new RefUtils<String>().getFirstMatch(client.compression_algorithms_server_to_client, server.compression_algorithms_server_to_client));
        ...
        try {
            np.lang_client_to_server = (new RefUtils<String>().getFirstMatch(client.languages_client_to_server, server.languages_client_to_server));
        }
        ...
        try {
            np.lang_server_to_client = (new RefUtils<String>().getFirstMatch(client.languages_server_to_client, server.languages_server_to_client));
        }
    ...
    private boolean establishKeyMaterial() {
        try {
            ...
            CipherEntry ce = (kxs.np.enc_algo_client_to_server);
            int enc_cs_key_len = ce.getKeysize();
            int enc_cs_block_len = kxs.np.enc_algo_client_to_server.getBlocksize();

            int mac_sc_key_len = MAC.getKeyLen(kxs.np.mac_algo_server_to_client);
            CipherEntry ce1 = (kxs.np.enc_algo_server_to_client);
            int enc_sc_key_len = ce1.getKeysize();
            int enc_sc_block_len = kxs.np.enc_algo_server_to_client.getBlocksize();
    ...
    public static void checkServerHostkeyAlgorithmsList(Collection<String> algos) {
