...
public class SFTPv3Client {
    ...
    private HashMap server_extensions = new HashMap();
     ...
     */
    private SFTPv3Client(Connection conn, PrintStream debug) throws IOException {
        ...
        if ((is == null)) throw new IOException("There is a problem with the streams of the underlying channel.");
    ...
    private void checkHandleValidAndOpen(SFTPv3FileHandle handle) throws IOException {
        ...
        if (handle.isClosed == true) throw new IOException("The file handle is closed.");
    }

    private void sendMessage(int type, int requestId, byte[] msg, int off, int len) throws IOException {
        int msglen = len + 1;

        if (type != Packet.SSH_FXP_INIT) msglen += 4;
    ...
    public String readLink(String path) throws IOException {
        ...
        if (t == Packet.SSH_FXP_NAME) {
            ...
            if (count != 1) throw new IOException("The server sent an invalid SSH_FXP_NAME packet.");
    ...
    private static String expandString(byte[] b, int off, int len) {
    ...
    private static byte[] createAttrs(SFTPv3FileAttributes attr) {
        TypesWriter tw = new TypesWriter();
    ...
    public void closeFile(SFTPv3FileHandle handle) throws IOException {
        if (handle == null) throw new IllegalArgumentException("the handle argument may not be null");
