 ...
 * @version $Id: Connection.java,v 1.33 2006/12/05 14:18:02 cplattne Exp $
 */
...
public class Connection {
     ...
     */
    public static synchronized CipherEntry[] getAvailableCiphers() {
        return CipherEntry.values();
    }
    ...
    final private CryptoWishList cryptoWishList = new CryptoWishList();
     ...
     * @throws IOException
     */
    private synchronized boolean authenticateWithKeyboardInteractive(String user, String[] submethods, InteractiveCallback cb) throws IOException {
        if (cb == null) throw new IllegalArgumentException("Callback may not ne NULL!");
    ...
    public synchronized boolean authenticateWithPassword(String user, String password) throws IOException {
        ...
        if (password == null) throw new IllegalArgumentException("password argument is null");
     ...
     * @throws IOException
     */
    private synchronized boolean authenticateWithPublicKey(String user, char[] pemPrivateKey, String password) throws IOException {
        if (tm == null) throw new IllegalStateException("Connection is not established!");
    ...
    public synchronized void addConnectionMonitor(ConnectionMonitor cmon) {
        if (cmon == null) throw new IllegalArgumentException("cmon argument is null");

        connectionMonitors.addElement(cmon);

        if (tm != null) tm.setConnectionMonitors(connectionMonitors);
    }
    ...
    private void close(Throwable t, boolean hard) throws IOException {
        if (cm != null) cm.closeAllChannels();
    ...
    public synchronized LocalPortForwarder createLocalPortForwarder(int local_port, String host_to_connect, int port_to_connect) throws IOException {
        ...
        if (!authenticated) throw new IllegalStateException("Cannot forward ports, connection is not authenticated.");
    ...
    public synchronized LocalStreamForwarder createLocalStreamForwarder(String host_to_connect, int port_to_connect) throws IOException {
        if (tm == null) throw new IllegalStateException("Cannot forward, you need to establish a connection first.");

        if (!authenticated) throw new IllegalStateException("Cannot forward, connection is not authenticated.");
    ...
    public synchronized void forceKeyExchange() throws IOException {
        if (tm == null) throw new IllegalStateException("You need to establish a connection first.");
    ...
    public synchronized String[] getRemainingAuthMethods(String user) throws IOException {
        if (user == null) throw new IllegalArgumentException("user argument may not be NULL!");
    ...
    public synchronized boolean isAuthMethodAvailable(String user, String method) throws IOException {
        if (method == null) throw new IllegalArgumentException("method argument may not be NULL!");

        String methods[] = getRemainingAuthMethods(user);

        for (int i = 0; i < methods.length; i++) {
            if (methods[i].compareTo(method) == 0) return true;
        }
    ...
    private SecureRandom getOrCreateSecureRND() {
        if (generator == null) generator = new SecureRandom();
    ...
    public synchronized Session openSession() throws IOException {
        ...
        if (!authenticated) throw new IllegalStateException("Cannot open session, connection is not authenticated.");
    ...
    public synchronized void sendIgnorePacket(byte[] data) throws IOException {
        if (data == null) throw new IllegalArgumentException("data argument must not be null.");
    ...
    /**
     * Removes duplicates from a collection.  preserves order.
     *
     * @param input a   array.
     * @return cleaned    .
     */
    private static Collection removeDuplicates(Collection input) {
     ...
     * @param ciphers
     */
    public synchronized void setClient2ServerCiphers(Collection ciphers) {
        if ((ciphers == null) || (ciphers.isEmpty())) throw new IllegalArgumentException();
        ciphers = removeDuplicates(ciphers);
        for (String cipherCandidate : (Collection<String>) ciphers) BlockCipherFactory.getEntry(cipherCandidate);
        cryptoWishList.c2s_enc_algos = (CipherEntry[]) ciphers.toArray(new CipherEntry[]{});
    }
     ...
     * @param macs
     */
    public synchronized void setClient2ServerMACs(Collection macs) {
        if ((macs == null) || (macs.isEmpty())) throw new IllegalArgumentException();
        macs = removeDuplicates(macs);
        MAC.checkMacList(macs);
        final Collection c2s_mac_algos = macs;
//        cryptoWishList.c2s_mac_algos = c2s_mac_algos;
    }
    ...
    public synchronized void setDHGexParameters(DHGexParameters dgp) {
        if (dgp == null) throw new IllegalArgumentException();
     ...
     * @param ciphers
     */
    public synchronized void setServer2ClientCiphers(Collection ciphers) {
        if (ciphers == null || ciphers.isEmpty()) throw new IllegalArgumentException();
        ciphers = removeDuplicates(ciphers);
        for (String cipherCandidate : (Collection<String>) ciphers) BlockCipherFactory.getEntry(cipherCandidate);
        final Collection s2c_enc_algos = ciphers;
//        cryptoWishList.s2c_enc_algos = s2c_enc_algos;
    }
     ...
     * @param macs
     */
    public synchronized void setServer2ClientMACs(Collection macs) {
        if ((macs == null) || (macs.isEmpty())) throw new IllegalArgumentException();

        macs = removeDuplicates(macs);
        MAC.checkMacList(macs);
        final Collection s2c_mac_algos = macs;
//        cryptoWishList.s2c_mac_algos = s2c_mac_algos;
    }
     ...
     *              at least one entry.
     */
    public synchronized void setServerHostKeyAlgorithms(Collection algos) {
        if (algos == null || algos.isEmpty()) throw new IllegalArgumentException();

        algos = removeDuplicates(algos);
        KexManager.checkServerHostkeyAlgorithmsList(algos);
        final Collection serverHostKeyAlgorithms = algos;
//        cryptoWishList.serverHostKeyAlgorithms = serverHostKeyAlgorithms;
    }
    ...
    public synchronized void requestRemotePortForwarding(String bindAddress, int bindPort, String targetAddress, int targetPort) throws IOException {
        ...
        if (!authenticated) throw new IllegalStateException("The connection is not authenticated.");
    ...
    public synchronized void setSecureRandom(SecureRandom rnd) {
        if (rnd == null) throw new IllegalArgumentException();
