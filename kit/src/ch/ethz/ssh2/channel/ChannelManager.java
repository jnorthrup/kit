...
public class ChannelManager implements MessageHandler {
    ...
    private static void waitForChannelSuccessOrFailure(Channel c) throws IOException {
        synchronized (c) {
    ...
    public void unRegisterX11Cookie(String hexFakeCookie, boolean killChannels) throws IOException {
        ...
        if (killChannels) {
            ...
            Vector<? extends Channel> channel_copy;

            synchronized (channels) {
                channel_copy = (Vector<? extends Channel>) channels.clone();
            }
            ...
            for (int i = 0; i < channel_copy.size(); i++) {
                ...
                synchronized (c) {
                    if (!hexFakeCookie.equals(c.hexX11FakeCookie)) continue;
                }
    ...
    public void sendData(Channel c, byte[] buffer, int pos, int len) throws IOException {
        while (len > 0) {
            ...
            synchronized (c.channelSendLock) {
                if (c.closeMessageSent) throw new IOException("SSH channel is closed. (" + c.getReasonClosed() + ")");
    ...
    public void registerThread(IChannelWorkerThread thr) throws IOException {
        synchronized (listenerThreads) {
            if (!listenerThreadsAllowed) throw new IOException("Too late, this connection is closed.");
     ...
     */
    public static int waitForCondition(Channel c, long timeout, int condition_mask) {
        long end_time = 0;
    ...
    public static int getAvailable(Channel c, boolean extended) {
        synchronized (c) {
            int avail;
    ...
    public int getChannelData(Channel c, boolean extended, byte[] target, int off, int len) throws IOException {
        ...
        if (increment > 0) {
            ...
            synchronized (c.channelSendLock) {
                ...
                if (!c.closeMessageSent) tm.sendMessage(msg);
            }
        }
